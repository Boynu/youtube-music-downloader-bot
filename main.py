...
async def start(message: types.Message,state: FSMContext):
	...
	if message.text == '/start':
		...
		plays=[]
		markup.row(types.KeyboardButton(f'ПЛЕЙЛИСТ ПО УМОЛЧАНИЮ [{check_numb(f"{userid}/ids.txt")}]'))
		plays.append('<code>ПЛЕЙЛИСТ ПО УМОЛЧАНИЮ</code>')
		dirs = os.listdir(f'{userid}/')
		dirs.remove('ids.txt')
		for name in dirs:
			...
			markup.row(types.KeyboardButton(f'{name[:-4]} [{check_numb(f"{userid}/{name}")}]'))
		dirss = '\n'.join(plays)
		await bot.send_message(message.chat.id, f'⚠️Внимание, вы получите список всех песен которые когда либо скачивали в этом боте.\nЕсли вы хотите использовать этого бота для прослушивания музыки, то нажмите на кнопку <i>"🧹Очистить историю чата"</i> и выберете плейлист\n\n<i>Возможно после очистки чата у вас пропадут кнопки, поэтому скопируйте заранее нужный вам плейлист:\n\n{dirss}</i>','HTML', reply_markup = markup)
		await go.get.set()
